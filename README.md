...
- [ ] 사용자로부터 경주할 자동차 이름을 입력받는다
    - [ ] 여러대의 자동차 이름을 입력한 경우 `,` 을 기준으로 구분한다
        - [ ] `,` 구분자 외에 다른 구분자를 사용한 경우 예외 처리 
    - [ ] 자동차 이름은 5자 이하만 가능하다
        - [ ] 이름을 5자 이상 입력한 경우 예외 처리
        - [ ] 이름을 입력하지 않은 경우 예외 처리
        - [ ] 중복된 이름을 입력한 경우 예외 처리
        - [ ] 이름에 공백이 포함된 경우 예외 처리

- [ ] 사용자로부터 시도할 횟수를 입력받는다
    - [ ] 숫자가 아닌 값이 들어온 경우 예외 처리
    - [ ] 숫자를 입력하지 않은 경우 예외 처리
    - [ ] 입력한 횟수가 1미만일 경우(음수) 예외처리

- [ ] 자동차 이동 거리 생성하고 자동차 전진 함수를 구현한다.
    - [ ] 전진하는 조건인 0에서 9 사이에서 무작위 숫자를 생성한다.
    - [ ] 생성된 값이 4이상일 경우 이동거리를 +1 한다.

- [ ] 자동차 전진시키고 차수별 실행 결과를 출력한다. 
- [ ] 우승자가 여러 명인 경우 `,` 로 구분하고, 한명일 경우는 그대로 출력한다
    - [ ] 우승자가 한명도 없는 경우 예외처리
